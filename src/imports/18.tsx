import imgFrame16 from "figma:asset/a10a1694f5ad1236f7ecc180fad802b8a32dce4f.png";
...
function Frame() {
  return (
    <div className="absolute content-stretch flex flex-col font-['Inter:Bold',sans-serif] font-bold gap-[30px] items-start leading-[normal] left-[51px] not-italic text-black top-[42px] w-[438px]">
      <p className="relative shrink-0 text-[24px] w-full">CMU Marathon</p>
      <p className="relative shrink-0 text-[16px] w-full">Our RFID solution to collect real time speed, location, and rank for all the runners. Auto location checkpoint sharing for rank status context and update on their social network. Dashboard screen for rank status with relevant online audience.</p>
    </div>
  );
}
...
function Number() {
  return (
    <div className="absolute bg-black bottom-[86.7%] box-border content-stretch flex gap-[10px] items-center justify-center left-0 px-[11px] py-[13px] right-[95.24%] rounded-[37.5px] top-0" data-name="number">
      <p className="font-['Inter:Bold',sans-serif] font-bold leading-[normal] not-italic relative shrink-0 text-[40px] text-nowrap text-white whitespace-pre">18</p>
    </div>
  );
}
...
function Frame1() {
  return (
    <div className="absolute bottom-0 left-[49.21%] right-0 rounded-[30px] top-0">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none rounded-[30px] size-full" src={imgFrame16} />
    </div>
  );
}
...
export default function Component() {
  return (
    <div className="relative size-full" data-name="18">
      <Description />
      <Number />
      <Frame1 />
    </div>
