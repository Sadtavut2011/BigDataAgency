import imgFrame24 from "figma:asset/da305133eccc894a6b9e675cd81b2a154f530b92.png";
...
function Frame() {
  return (
    <div className="absolute content-stretch flex flex-col font-['Inter:Bold',sans-serif] font-bold gap-[30px] items-start left-[51px] not-italic text-black top-[42px] w-[438px]">
      <p className="leading-[normal] relative shrink-0 text-[24px] w-full">BANK VIP SERVICE</p>
      <div className="leading-[normal] relative shrink-0 text-[16px] w-full">
        <p className="mb-0">{`> Queue management for banking staff`}</p>
        <p className="mb-0">{`> User experience feedback`}</p>
        <p>{`> Online queuing service and postpone a queue feature`}</p>
      </div>
    </div>
  );
}
...
function Number() {
  return (
    <div className="absolute bg-black bottom-[86.7%] box-border content-stretch flex gap-[10px] items-center justify-center left-0 px-[11px] py-[13px] right-[95.24%] rounded-[37.5px] top-0" data-name="number">
      <p className="font-['Inter:Bold',sans-serif] font-bold leading-[normal] not-italic relative shrink-0 text-[40px] text-nowrap text-white whitespace-pre">26</p>
    </div>
  );
}
...
function Frame1() {
  return (
    <div className="absolute bottom-0 left-[49.21%] right-0 rounded-[30px] top-0">
      <img alt="" className="absolute inset-0 max-w-none object-50%-50% object-cover pointer-events-none rounded-[30px] size-full" src={imgFrame24} />
    </div>
  );
}
...
export default function Component() {
  return (
    <div className="relative size-full" data-name="26">
      <Description />
      <Number />
      <Frame1 />
    </div>
